...
//            externalApiConnector.autoApprovalOnboarding(message.getInstitution().getTaxCode(), institutionProductsAllowedMap.get().get(message.getProduct()), request);
            log.debug("Validation result: {}", validateTestingProduct(message));
        }
        ...
        if (institutionProductsAllowedMap.isPresent() && institutionProductsAllowedMap.get().containsKey(message.getProduct())) {
            for (String productId : institutionProductsAllowedMap.get().get(message.getProduct())) {
                ...
                if (!TESTING.equals(product.getStatus())) {
                    log.error("[ProductStatus - Error]Product {} no longer in TESTING, onboarding not enabled", message.getProduct());
                    return false;
                }
            }
        } else {
            log.error("[Test - Onboarding - Error]No Testing products available for {}, onboarding-request = {}", message.getProduct(), message);
            return false;
        }
        return true;
    }
